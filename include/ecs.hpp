...
namespace mgm {
    struct HierarchyNode {
         ...
         */
        std::vector<MGMecs<>::Entity> children() const { return {begin(), end()}; }
    ...
    template<typename T> struct SerializedData {
      private:
        JObject json{};
      ...
      public:
        ...
        template<typename I> decltype(std::declval<JObject>().operator[](std::declval<I>())) operator[](I i) { return json[i]; }
        template<typename I> decltype(std::declval<const JObject>().operator[](std::declval<I>())) operator[](I i) const {
            return json[i];
        }
    ...
    template<typename, typename = void> struct has_inspect : std::false_type {};

    template<typename T>
    struct has_inspect<T, std::void_t<decltype(std::declval<T>().inspect())>>
        : std::is_same<bool, decltype(std::declval<T>().inspect())> {};

    template<typename T> inline constexpr bool has_inspect_v = has_inspect<T>::value;


    template<typename, typename = void> struct has_external_inspect : std::false_type {};

    template<typename T>
    struct has_external_inspect<T, std::void_t<decltype(inspect(std::declval<T&>()))>>
        : std::is_same<bool, decltype(inspect(std::declval<T&>()))> {};

    template<typename T> inline constexpr bool has_external_inspect_v = has_external_inspect<T>::value;
#endif
    ...
    struct has_serialize<T, std::void_t<decltype(std::declval<const T&>().serialize())>>
        : std::is_same<SerializedData<T>, decltype(std::declval<const T&>().serialize())> {};

    template<typename T> inline constexpr bool has_serialize_v = has_serialize<T>::value;


    template<typename, typename = void> struct has_external_serialize : std::false_type {};

    template<typename T>
    struct has_external_serialize<T, std::void_t<decltype(inspect(std::declval<const T&>()))>>
        : std::is_same<SerializedData<T>, decltype(serialize(std::declval<const T&>()))> {};

    template<typename T> inline constexpr bool has_external_serialize_v = has_external_serialize<T>::value;


    template<typename, typename = void> struct has_deserialize : std::false_type {};

    template<typename T>
    struct has_deserialize<T, std::void_t<decltype(std::declval<T&>().deserialize(std::declval<const SerializedData<T>&>()))>>
        : std::is_same<void, decltype(std::declval<T&>().deserialize(std::declval<const SerializedData<T>&>()))> {};

    template<typename T> inline constexpr bool has_deserialize_v = has_deserialize<T>::value;


    template<typename, typename = void> struct has_external_deserialize : std::false_type {};

    template<typename T>
    struct has_external_deserialize<
        T, std::void_t<decltype(deserialize(std::declval<T&>(), std::declval<const SerializedData<T>&>()))>>
        : std::is_same<void, decltype(deserialize(std::declval<T&>(), std::declval<const SerializedData<T>&>()))> {};

    template<typename T> inline constexpr bool has_external_deserialize_v = has_external_deserialize<T>::value;


    class EntityComponentSystem : public System {
        template<typename T> friend struct SerializedData;
      ...
      public:
        ...
        std::unique_lock<std::mutex> ecs_lock() { return std::unique_lock{mutex}; }
         ...
         * @param enable_as_raw_component If true, the type will be allowed to be placed as a raw component directly on an
         * entity in the heirarchy
         */
        template<typename T>
         ...
         */
        template<typename T> JObject serialize_component(const MGMecs<>::Entity entity) {
            std::unique_lock lock{mutex};
            return serialized_types[types_unique_ids[typeid(T).hash_code()]].serialize(entity);
        }

        /**
         * @brief Deserialize a component from some json data, and set its value, or add it to the entity if it doesn't exist
         * already
         *
         ...
         */
        template<typename T> void deserialize_component(const MGMecs<>::Entity entity, const JObject& json) {
            std::unique_lock lock{mutex};
         ...
         */
        template<typename T> std::string type_unique_identifier() const {
            std::unique_lock lock{mutex};
        ...
        /**
         * @brief Load components which are a registered type from Json, and add them to the entity, or set the existing
         * component on the entity to the new deserialized value
         *
        ...
        /**
         * @brief Dump a Hierarchy Node into json (its name, all of its components, and all of its children, all the way down
         * the hierarchy)
         *
         ...
         *
         * @param entity The entity to make into the new root of the tree to deserialize (can be non-root, new tree will pe
         * placed under the given antity anyway)
        ...
        ~EntityComponentSystem() {
            std::unique_lock lock{mutex};
            ecs.destroy(root);
            for (const auto& [scene_path, scene_root] : editable_scenes) ecs.destroy(scene_root);
        }
    };

    template<typename T> SerializedData<T>::operator JObject() {
        MagmaEngine engine{};
